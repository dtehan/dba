...
- **Global scope**: `/Users/Daniel.Tehan/Code/dba/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/Daniel.Tehan/Code/dba/.claude/notes/`)
...
2. Glob `/Users/Daniel.Tehan/Code/dba/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/Daniel.Tehan/Code/dba/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/Daniel.Tehan/Code/dba/.claude/notes/` — works in any directory
