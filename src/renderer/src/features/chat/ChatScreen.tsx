...
function buildSystemPrompt(agents: Array<{ name: string; description: string; category: string }>): string {
  if (agents.length === 0) return BASE_SYSTEM_PROMPT;
...
export function ChatScreen(): JSX.Element {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const result = await api.sendChat(messagesForApi, buildSystemPrompt(agents));
