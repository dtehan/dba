...
## Query Activity Dashboard Redesign

### Dashboard Layout & Data

- **QADH-01**: Query Activity displays data in a proper table with columns: Username, CPU Time, I/O Count, Elapsed Time, Start Time, SQL Text (~100 char truncated)
- **QADH-02**: Table supports sortable column headers with arrow indicators; CPU and I/O sort trigger server-side re-fetch with new ORDER BY clause
- **QADH-03**: Time range preset filter buttons: Last 1 hour, Last 4 hours, Today (default), Yesterday, Last 7 days — each triggers server-side re-fetch with WHERE clause on StartTime
- **QADH-04**: Text filter for SQL text and username search (existing, retained)

### SQL Detail Panel

- **QADH-05**: Clicking SQL text opens a right-side slide-out panel (~40% width) showing full SQL with syntax highlighting and copy button
- **QADH-06**: Slide-out panel includes metadata header: username, CPU time, I/O count, elapsed time, start time
- **QADH-07**: Panel auto-closes when sort column or time range filter changes

### Panel Actions

- **QADH-08**: "Explain Plan" button in panel runs EXPLAIN on the SQL against Teradata and displays output in the panel
- **QADH-09**: "Analyze" button launches the SQL optimization subagent (query-performance) with the full SQL, navigating to chat to show streaming results
...
| QADH-01 | Phase 5 | Planned |
| QADH-02 | Phase 5 | Planned |
| QADH-03 | Phase 5 | Planned |
| QADH-04 | Phase 5 | Planned |
| QADH-05 | Phase 5 | Planned |
| QADH-06 | Phase 5 | Planned |
| QADH-07 | Phase 5 | Planned |
| QADH-08 | Phase 5 | Planned |
| QADH-09 | Phase 5 | Planned |

**Coverage:**
- v1 requirements: 44 total (complete)
- v1.1 requirements: 9 total (Phase 5)
- Mapped to phases: 53
- Unmapped: 0 ✓
